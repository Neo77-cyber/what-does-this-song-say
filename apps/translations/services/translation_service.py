...
def process_song_translation(track_name, artist_name):
    ...
    try:
        ...
        return {
            ...
            'translated': "The lyrics source is taking a nap. Please try again in a few seconds!",
            'status': 'error'
        }
    
    if not song or not song.lyrics:
        logger.warning(f" Lyrics not found on Genius for: {track_name}")
        return {
            'original': "No Lyrics Found",
            'translated': "I searched high and low but couldn't find the lyrics for this specific track.",
            'status': 'error'
        }
